...
* __Using RMarkdown__
* __Basic data visualization in Base R and ggplot__
</font>

## Troubleshooting

Likely the most important skill to have under your belt for coding in R is how to troubleshoot. Regardless of your experience level with R, you are bound to run into issues in the form of error messages or undesirable outputs. Luckily, the internet exists. Not only does the internet exist, but the internet contains, within it, practically endless resources regarding R. All one has to do is copy and paste an error message into a search engine "[x topic] in r" and they're bound to find forums and blog posts dedicated to their issue. Further, within recent years, the development of large language models (LLMs) such as ChatGPT have dramatically changed how many people troubleshoot coding issues (see [this blog post](https://www.zdnet.com/article/how-to-use-chatgpt-to-write-code-what-it-does-well-and-what-it-doesnt/) for a description of how you might use ChatGPT to help you code). We heavily encourage the use of the internet, including LLMs, to aid in your coding for this class.
...
__Flip back to the .Rmd file and identify where you see the following formatting syntax (outside of a chunk of code):__

- __text bookmarked with two underscores__
- __text in square brackets followed by text in parentheses__
- __hashtags followed by text__
...
Another note on troubleshooting: you can learn more about a function by searchin for it on some search engine, or by typing in the a `?` followed by the name of the function in the console (for example, type into the console: `?sum`). The function documentation should open in the help tab, and you can find out what the function does, what package it is contained in, and which arguments it takes. 
...
Often, it is helpful to write out an example of what you would like to do before you try to write your function. Once you've coded up an example, you should 1) identify which elements of your code you'd like to be customizable in the function, 2) replace those elements with short but informative indexes (these will become the names of the arguments within your function), and 3) wrap it all in the `function()` function with the index names as your arguments.
...
__Use these averages to create a new variable within the dataframe for the approximate seed output of each plant. Because the average seeds/fruit is dependent on the nutrient treatment, you may need to use some sort of `ifelse` statement or the `case_when()` function (take a moment to search these methods if you are not already familiar).__
...
__Make a bar plot of average seed output under each nutrient treatment.__
...
__Make a scatter plot to compare seed output and stem height, but include a best fit line (you may need to search how to do this) for each of the three nutrient treatments. Color the points and lines by treatment.__
